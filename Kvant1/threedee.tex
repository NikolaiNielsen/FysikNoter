...
\begin{document}
	...
	\begin{equation}
		\hs{S} \ket{s,m} = \hbar\sqrt{s(s+1)-m(m\pm1)} \ket{s,m\pm 1}.
	\end{equation}
	Hvor $ \hs{S} \equiv \op{S}_x\pm i \op{S}_y $. Det viser sig, at de tilladte værdier af $ s $ er helt fundamentale til partiklerne, og disse kan da ikke ændres. Eksempelvis har pi mesoner spin 0, elektroner (samt protoner og neutroner) har spin 1/2, fotoner spin 1 og så videre
	...
	\end{equation}
	der kaldes for \textbf{Pauli spin-matricerne}. Det bemærkes, at $ \op{S}^2 $ og $ \op{\V{S}} $ alle er hermitiske, og dermed observerbare, mens $ \hs{S} $ ikke er det. Egenspinorerne til $ \op{S}_z $ er
	\begin{equation}
	...
	\subsection{Sfæriske harmonier, Laguerrepolynomier, radielle bølgefunktioner}
	\subsubsection{Sfæriske harmonier}	
	\begin{table}[H]
		\centering
		...
		\end{tabular}
	\end{table}
	\subsubsection{Radielle bølgefunktioner}
	\begin{table}[H]
		\centering
	...
	\subsubsection{Laguerrepolynomier}
	\begin{table}[H]
		\centering
		\begin{tabular}{>{\vspace{0.2cm}$\displaystyle} l <{$}}
			L_0 = 1 \\
		...
		\end{tabular}
	\end{table}
	\subsubsection{Associerede Laguerrepolynomier}
	\begin{table}[H]
		\centering
