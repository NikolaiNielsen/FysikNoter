...
\begin{document}
	...
	\begin{equation}
		\brac{j} = \frac{\sum_0^{\infty} j N(j)}{N} = \sum_{j = 0}^{\infty} j P(j)
	\end{equation}
	...
	\begin{align} 
		...
		\brac{p} &= \infint \Psi\konj \pp{\frac{\hbar}{i} \diff{}{x}}\Psi \ud x.
	\end{align}
	Det, inde i parentesen i integralet kaldes for en \textbf{operator}, og operatoren $ \op{x} = x $ siges at ">repræsentere"< position, og operatoren $ \op{p} = -i\hbar \partial/\partial x $ siges at ">repræsentere"< impulsen. \textbf{I kvantemekanik regnes alle relevante størrelser altså ud, ved at tage størrelsens repræsentative operator, og smide den ind mellem $ \Psi\konj $ og $ \Psi $ i det uendelige integral.}
	...
	\end{equation}
	For at regne forventningsværdien af en given størrelse (operator) $ \op{Q}(x,p) $, skal alle $ p $'er bare erstattes med $ \op{p} = -i\hbar (\partial / \partial x) $, og så skal udtrykket smides ind i midten af integralet, og hele skidtet integreres:
	\begin{equation}
		\brac{\op{Q}(x,p)} = \infint \Psi\konj \op{Q}\pp{x, -i \hbar \diff{}{x}} \Psi \ud x.
	\end{equation}
	Og i tilfældet af den kinetiske energi, er forventningsværdien
	\begin{equation}
		\brac{\op{T}} = \frac{-\hbar^2}{2m} \infint \Psi\konj \diff{^2 \Psi}{x^2} \ud x.
	\end{equation}
	...
	\begin{equation}
		\brac{\op{p}} = m \diff[\ud]{\brac{x}}{t}
	\end{equation}
	Ligeledes kan den kinetiske energi også regnes nemt:
	\begin{equation}
		\brac{\op{T}} = \frac{\brac{p^2}}{2 m}
	\end{equation}
	...
	\begin{equation}
		\diff[\ud]{\brac{\op{p}}}{t} = \brac[\bigg]{-\diff{V}{x}}
	\end{equation}
	...
	\begin{equation}
		\sigma_x^2 = \brac{x^2}-\brac{x}^2, \quad \sigma_p^2 = \brac{p^2}-\brac{p}^2
	\end{equation}
	\begin{equation}
