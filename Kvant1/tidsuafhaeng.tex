...
\begin{document}
	...
	\end{equation}
	Produktet $ \saenk\haev$ er
	\begin{equation}
		\saenk\haev = \frac{1}{2 \hbar m \omega} [\op{p}^2 + (m\omega \op{x})^2 - im\omega (\op{x}\op{p}-\op{p}\op{x})]
	\end{equation}
	Den sidste parentes i parentesen er den kanoniske kommutator, og produktet kan da skrives ved denne:
	\begin{align}
		\saenk\haev &= \frac{1}{2 \hbar m \omega} [p^2+(m\omega \op{x})^2] + \frac{1}{2i\hbar} [\op{x},\op{p}] \\
		&= \frac{1}{\hbar \omega} \op{H} + \frac{1}{2}
	\end{align}
	På lige vis fås produktet $ \haev \saenk $
	\begin{equation}
		\haev\saenk = \frac{1}{\hbar \omega} \op{H} - \frac{1}{2}
	\end{equation}
	Dette giver da kommutatoren $ [\haev,\saenk] = 1 $, og $ \op{H} $ kan skrives ved
	\begin{equation}
		\op{H} = \hbar \omega \pp{\saenk\haev-\frac{1}{2}} =  \hbar \omega \pp{\haev\saenk+\frac{1}{2}}.
	\end{equation}
	og Schrödingerligningen er da
	\begin{equation}
		\op{H} \psi = \hbar \omega \pp{\op{a}_{\pm}\op{a}_{\mp}\pm \frac{1}{2}} \psi = E\psi.
	\end{equation}
	Så nu kan Schrödingerligningen opskrives ved \textit{to} operatorer, i stedet for kun én. Woop de fucking do! Men vent, for vi har jo ikke navngivet $ \haev $ og $ \saenk $ endnu! Vi kalder dem nemlig for henholdsvis \textit{hæve}- og \textit{sænke}-operatorerne (I bet you didn't see that coming). De har dette navn, fordi det viser sig, at hvis $ \op{H}\psi = E\psi $, så er $ \op{H}\haev\psi = (E+\hbar \omega) \psi$ og $ \op{H}\saenk \psi = (E-\hbar \omega) \psi $. De to operatorer \textbf{hæver} og \textbf{sænker} altså energiniveauet af en given løsning! Men vi er jo stadig ikke tættere på nogen løsning. Nu ved vi bare at der er flere end én...
	...
	\begin{equation}
		\saenk \psi_0 = 0. \quad \text{(læg mærke til nul-indekseringen)}
	\end{equation}
	...
	\end{equation}
	Denne tilstand har energien $ E_0 = \frac{1}{2}\hbar\omega $. Hvilket man får fra Schrödingerligningen og at $ \saenk\psi_0 = 0 $. Hermed er det bare at bruge hæveoperatoren for at få alle de næste tilstande.
	\begin{equation}
		\psi_n = A_n (\haev)^n \psi_0, \quad A_n = \frac{1}{\sqrt{n!}}, \quad E_n = \pp{n+\frac{1}{2}} \hbar \omega.
	\end{equation}
	Normaliseringsfaktoren $ A_n $ kan fås ud fra den egenskab at $ \op{a}_{\pm} \psi_n \propto \psi_{n\pm 1} $ og at $ \haev $ er den \textbf{hermitisk konjugerede} til $ \saenk $ (og omvendt):
	\begin{equation}
		\infint f\konj (\op{a}_{\pm} g) \ud x = \infint (\op{a}_{\mp} f)\konj g \ud x
	\end{equation}
	Ud fra dette, fås også følgende nyttige egenskaber
	\begin{equation}
		\haev \psi_n = \sqrt{n+1}\, \psi_{n+1}, \quad \saenk \psi_n = \sqrt{n} \,\psi_{n-1}.
	\end{equation}
	...
	\begin{equation}
		\Psi (x,0) = \sum_{n=0}^{\infty} c_n \psi_n = \sum_{n=0}^{\infty} c_n \frac{1}{\sqrt{n!}} (\haev)^n \psi_0, \quad c_n = \infint \psi_n(x)\konj \Psi(x,0) \ud x
	\end{equation}
	...
	\begin{align}
		\op{x} &= \sqrt{\frac{\hbar}{2m\omega}} (\haev+\saenk),\\
		\op{p} &= i \sqrt{\frac{\hbar m \omega}{2}} (\haev-\saenk),\\
		\op{x}^2 &= \frac{\hbar}{2m\omega} \bb{\haev^2 + \haev \saenk + \saenk \haev + \saenk^2}, \\
		\op{p}^2 &= \frac{-\hbar m \omega}{2} \bb{\haev^2 - \haev \saenk - \saenk \haev + \saenk^2}.
	\end{align}
