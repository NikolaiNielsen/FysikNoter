...
\begin{document}
	...
	\end{align*}
	Et sådan ligningssystem kan også beskrives ved matricer, hvor matricen $ \mat{A} $ kaldes for \textbf{koefficientmatricen}, og søjlematricen $ \mat{B} $ kaldes for \textbf{konstantsøjlen}:
	\begin{equation*}
		\mat{A}= \begin{pmatrix}
		...
		\end{pmatrix}, \qquad \mat{B}=\begin{pmatrix}
		b_1 \\ \vdots \\ b_m
	\end{pmatrix},
	\end{equation*}
	Blokmatricen $ \mat{C}= \begin{pmatrix}
	\mat{A} & \mat{B}
	\end{pmatrix} $ kaldes for ligningssystemets \textbf{totalmatrix}, og har formen:
	\begin{equation*}
	...
	\subsubsection*{Løsning af lineære ligningssystemer}
	...
	Og omvendt:
	\begin{enumerate}
		\item Hvis ligningssystemet $ \mat{A}\,\mat{X}=\mat{B} $ har mindst én løsning for hvert valg af $ \mat{B} $, da er $ d=m $. ($ f $ er surjektiv)
		\item Hvis ligningssystemet $ \mat{A}\,\mat{X}=\mat{0} $ kun har én løsning (nemlig $ \mat{0} $), da er $ d=n $ ($ f $ er injektiv)
		\item Hvis ligningssystemet $ \mat{A}\,\mat{X}=\mat{B} $ har netop én løsning for hvert valg af $ \mat{B} $, da er $ d=m=n $. ($ f $ er bijektiv)
	\end{enumerate}
	
	For at bestemme in-/sur-/bijektivitet kan matricen omdannes til en trappematrix med $ d $ trin. Der er følgende sammenhæng mellem antal trin, rækker og søjler:
	\begin{enumerate}
		\item Hvis $ d=n $ (\#trin = \#søjler), så er $ f $ injektiv
		\item Hvis $ d=m $ (\#trin = \#rækker), så er $ f $ surjektiv
		\item Hvis $ d=m=n $ (\#trin = \#rækker = \#søjler), så er $ f $ bijektiv
	\end{enumerate}
	\subsubsection*{Lineære ligningssystemer og lineære afbildninger}
	Et ligningssystem af typen $ \mat{A}\,\mat{X}=\mat{B} $ kan også beskrives ved lineære afbildninger. Hvis vi lader $ \morffFF{n}{m} $ være den lineære afbildning, der hører til $ \mat{A} $, sætter $ \V{x}=\mat{X}, \ \V{b}=\mat{B} $, så bliver ligningssystemet
	\begin{equation}
		f(\V{x})=\V{b}
	\end{equation}
	For lineære afbildninger gælder følgende om injektivitet:
	\begin{itemize}
		\item En lineær afbildning $ f $ er injektiv når ligningen $ f(\V{x})=\V{o} $ kun har løsningen $ \V{x}=\V{o} $
	\end{itemize}
	...
	\subsubsection{Løsning med determinanter (Cramers formler)}
	...
	\end{equation}
	Hvor den øverste determinant er matricen $ \mat{A} $, hvor den $ j $'te søjle er udskiftet med konstantsøjlen $ \V{b} $. Eksempelvis er $ x_3 $ til $ n=3 $:
	\begin{equation}
