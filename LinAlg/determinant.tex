...
\begin{document}
	...
	\subsection{Determinanter for 2x2 og 3x3 matricer}
	...
	\subsection{Determinant af vilkårlig matrix}
	...
	\subsection{Determinant af en endomorfi}
	En \textbf{endomorfi} er en afbildning $ \morf{f}{V}{V} $ på sig selv. Denne er da givet ved en kvadratisk matrix ($ n\times n $, hvis $ \dim V = n $). Hvis $ \bas{A}= (\V{a}_1, \dots , \V{a}_n) $ er en basis for V er determinanten $ \det f $  af endomorfien $ \morf{f}{V}{V} $ defineret som $ \det \transf{\bas{A}}{f}{\bas{A}} $. Denne afhænger da ikke af basen, og hvis $ \bas{A}' $ er en ny basis i V, er $ \det \transf{\bas{A}'}{f}{\bas{A}'} = \det \transf{\bas{A}}{f}{\bas{A}} $. \textbf{Determinanten af en matrix er altså ens, lige meget hvilken basis denne er repræsenteret i.}
\end{document}
